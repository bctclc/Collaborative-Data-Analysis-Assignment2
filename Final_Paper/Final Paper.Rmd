...
The explanatory variables will be each person's demographic characteristics such as age, gender, education level, etc.
...
grp <- c('Sex', '', 'Income', '', '', '', 'College degree', '', 
         'Drivers licence', '', 'Childrean in family', '')
...
People who have a college degree, people who have a drivers licence, and people who have children in their family shows grater interests compared to people who do not respectively. 
...
* **DVHsize**: Mean=`r round(mean(EVINTEREST$DVHsize), digits=2)`, Standard deviation=`r round(sd(EVINTEREST$DVHsize), digits=2)`. The distribution of household size is shown in figure 5 below.
...
Where $p$ is the probability of being interested in EVs. The estimated model is shown in table 5.
...
As for encouraging cost-related factors shown in figure 10, the top three choices are the same, only with a different order. While the top choice is still the purchase cost (`r round(mean(EVINTEREST$ENCpurchase)*100, digits=2)`%), the second is fuel/recharging cost (`r round(mean(EVINTEREST$ENCfuel)*100, digits=2)`%) and the third is maintenance cost (`r round(mean(EVINTEREST$ENCmaintenance)*100, digits=2)`%).

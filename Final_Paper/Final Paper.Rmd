...
This study will focus on EVs, which are the most recent generation of low emission vehicles and thus not many researches have been done so far, compared to other types of sustainable vehicles.
...
We are interested in the potentially different attitudes and priorities of different groups and in identifying potential buyers of EVs as well as obstacles preventing EV consumption.
In the survey, the questionnaire asks the respondent's attitudes towards buying an EV. The answer choices ranges from "I already have one" to "I don't drive". 
The answers are recoded into a binary dummy variable whether the person is interested in buying an EV ("I already have one," "I am thinking about buying one," "I thought about buying one") or not.
The explanatory variables will be each person's demographic characteristics such as age, gender, education level, and etc.
In order to take the regional differences in policies to promote EVs into account, if there are, we will also include government administrative regions within the U.K. as a control.


As our dependent variable is a binary dummy, **logistic regression** expressed in the equation below will be adopted:

$$\ln{\frac{p}{1-p}}=\beta_{0} + \beta_{1} x_{1i} + \beta_{2} x_{2i} + \dots + \beta_{k} x_{ki}$$

Where $p$ is the probability that the dependent variable equals to 1 (i.e. being interested in buying an EV). By exponentiating the coefficients ($e^{\beta_{i}}$), we will obtain the odds ratio for different group of people.

Further, by converting the log odds ($\ln{\frac{p}{1-p}}$) into $p$ as shown in the equation below, we will obtain the predicted probability for the *i*th person depending on his/her personal characteristics. 

$$p=\frac{1} {1 + exp\{-(\beta_{0} + \beta_{1} x_{1i} + \beta_{2} x_{2i} + \dots + \beta_{k} x_{ki})\}}$$

Thus, we are expecting to see different predicted probabilities of being interested in buying an EV for different demographic groups, which will provide some useful information for the policy makers to create efficient means to promote EVs.
...
Descriptions <- c("Whether the respondent is interested in EVs or not", 
                  "Respondent's age",
                  "Respondent's sex",
                  "Respondent's income in four categories (low, low-mid, high-mid, high)",
                  "Whether the respondent is a college graduate or not",
                  "Whether the respondent has a valid driver's licence or not",
...
The original data set contains 962 observations from Feburary 2014 and 1,034 from Feburary 2015. 
After combining the two samples and liminating all the observations with missing values in any of the key variable listed above, a subset of 1,710 observations is obtained for this study. 

Among the whole sample, `r round(mean(EVINTEREST$EVinterest)*100, digits=2)` % of people have interests in EVs. 
However, one can easily expecte, the percentage is not uniform among different groups of people. 
Table 2 below is the percentage of interested peoope by groups. 

```{r echo=F, warning=F, message=F, error=F, results='asis'}
# table
grp <- c('Sex', '', 'Income', '', '', '', 'College degree', '', 
         'Driver's licence', '', 'Children in family', '')
sgrp <- c('Male', 'Female', 'Low', 'Lower-middle', 'Higer-middle',
...
cbd_grp <- data.frame(category=grp, group=sgrp, percentage=round(meangrp*100, digits = 2))
kable(cbd_grp, caption="Percentage of people interested in EVs by groups")
```

From this, some possible characteristics of potential EV buyers can be observed: males generally have greater interested in EVs than females. 
Higher income people are more likely to be interested. 
People who have a college degree, people who have a driver's licence, and people who have children in their family shows grater interests compared to people who do not respectively. 
These results make intuitive sense, and conforms the findings in previous studies.

Besides these groups, figure 1 below shows the geographical distribution by Government Administration Regions. People living in southern part of the U.K. have higher interests in EVs.
These heterogeneity suggests that the U.K. government may want to differentiate the strategies to promote further EV adoptions.
Detailed determinants of people'S interest in EVs will be further investigated in the next section. 

```{r echo=F, warning=F, message=F, error=F, results='asis', fig.cap="Percentage of interested people by region"}
...
The sample is summarized as follows by the other variables:

* **RAGE**: Mean age of the sample is `r round(mean(EVINTEREST$RAGE), digits=2)`, with Standard deviation of `r round(sd(EVINTEREST$RAGE), digits=2)` years.
The distribution of age is shown in figure 2 below.

* **Male**: `r round(mean(EVINTEREST$Male)*100, digits=2)` % of respondents are male.

* **inccat**: distribution is shown in table 3 and figure 3 below. 
...
* **licence**: `r round(mean(EVINTEREST$licence)*100, digits=2)` % of respondents have valid driver's licence

* **NumCar**: Mean number of cars in household is `r round(mean(EVINTEREST$NumCar), digits=2)`, with standard deviation of `r round(sd(EVINTEREST$NumCar), digits=2)`. The distribution of the number of cars in respondent's household is shown in figure 4 below.

* **DVHsize**: The mean is `r round(mean(EVINTEREST$DVHsize), digits=2)`, and the standard deviation is `r round(sd(EVINTEREST$DVHsize), digits=2)`. The distribution of household size is shown in figure 5 below.
...
mtext(side=1, "Gross Annual Income (pounds)", line=2)
...
We only include socio-economic variables in our base model (regression 1) and add in a new group of independent variables each time. As the results in table 4 show, all models are significant at 99% level. Except for the two variables, size of household and having dependent children or not, all other variables are statistically significant as well. The fact that household-related variables are effective predictors at macro level, but not at micro level, may suggest a neighborhood effect, but it is beyond the scope of this paper.
...
labels <- c('Age', 'Male', 'Income: low-middle', 'Income: high-middle', 'Income: high', 
            'College degree', 'Driver's licence', 'Number of cars', "Size of household", 
...
The estimated model is shown in table 5.

```{r echo=F, warning=F, message=F, error=F, results='asis'}
finallabels <- c('Age', 'Male', 'Income: low-middle', 'Income: high-middle', 'Income: high', 
            'College degree', 'Driver's licence', 'Number of cars', 'Scotland', '(Intercept)') 
...
Having a college degree or driver's license also increases the likelihood of being interested: the odds of having interested in EVs are 92% higher ($e^{0.65}=1.92$) for people with college degree compared to people without [z=4.38, p<0.001], the odds are 148% higher ($e^{0.91}=2.48$) for those who have driver's licence compared to those who don't [z=3.83, p<0.001]. 
...
Figure 7 below shows the major obstacles to buying an EV. 
...
Now we turn to the factors that may encourage EV consumption. The choices provided in the survey and their relative importance in the respondents' minds are shown in the figure 8.
...
As shown in figure 9 below, `r round(mean(EVINTEREST$POCpurchase)*100, digits=2)`% of respondents think that the purchasing costs would put them off buying an EV. The other major concerns are the maintenance costs (`r round(mean(EVINTEREST$POCmaintenance)*100, digits=2)`%) and the fuel/recharging costs including fuel tax (`r round(mean(EVINTEREST$POCfuel)*100, digits=2)`%). 
...
As for encouraging cost-related factors shown in figure 10, the top three choices are the same, only with a different order. While the top choice is still the purchase costs (`r round(mean(EVINTEREST$ENCpurchase)*100, digits=2)`%), the second is fuel/recharging costs (`r round(mean(EVINTEREST$ENCfuel)*100, digits=2)`%) and the third is maintenance costs (`r round(mean(EVINTEREST$ENCmaintenance)*100, digits=2)`%).
...
In an attempt to formulate policy recommendations that resemble differentiated marketing strategy, we compare the relative weight given to the factors by different groups. The results are shown in figure 11 (general encouragement) and figure 12 (cost-related encouragement). We find that in most cases, different groups share their relative weight given to different factors. However, we do find some differences that can be used in differentiated marketing strategy.
...
![Relative weights to factors that encourage buying EVs among groups](./Rplot.png)

![Relative weights to cost-related factors that encourage buying EVs among groups](./Rplot01.png)
...
---
notice: | 
  @CiteR,@R-captioner,@R-dplyr,@R-foreign,@R-ggmap,@R-ggplot2,@R-googleVis,@R-knitr,@R-maptools,@R-markdown,@R-plyr,@R-RColorBrewer,@R-repmis,@R-rms,@R-stargazer,@R-Zelig
...

...
kable(cbd_grp, caption="% of people interested in EVs by groups")
```
...
Besides these groups, the figure below shows the geographical distribution by Government Administration Regions. People living in southern part of the U.K. have higher interests in EVs.
...
```{r echo=F, warning=F, message=F, error=F, results='asis', fig.cap="% of interested people by region"}
...
* **inccat**: distribution is shown in table 3 and figure 2 below. 
...
* **NumCar**: Mean number of cars in household is `r round(mean(EVINTEREST$NumCar), digits=2)`, with standard deviation of `r round(sd(EVINTEREST$NumCar), digits=2)`. The distribution of the number of cars in respondent's household is shown in figure 3 below.
